...
(Use pseudocode to illustrate KEY CHANGES only. Keep it highly concise and abstract:
- Expand method body with minimal pseudo-logic inside
- Focus on WHAT changes, not detailed implementation
- Use `// ...` to skip unchanged parts)

### 5.1 Key Changes
```
// NEW
UserService.validateQuota(userId) {
    quota = getQuota(userId)
    if quota.exceeded -> throw QuotaError
}

// MODIFIED
OrderProcessor.submit(order) {
    // ...
    queue.pushAsync(order)  // Changed: sync -> async
...
PaymentHandler.process(payment) {
    ...
    if success -> notify(user) + updateCache()
    if failure -> retry(3x) -> fallback(manualReview)
}
```
